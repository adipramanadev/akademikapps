...
class IndexApiController extends BaseController{
	...
	private function login_user($username, $password_text, $rememberme = false){
		...
		if(!empty($user)){
			...
			if(password_verify($password_text, $password_hash)){
				...
				set_session("user_data", $user); // Set user data in session
	
				if($rememberme){
					$sessionkey = time().random_str(20);
					$db->where("id", $user['id']);
					$res = $db->update($tablename, ["login_session_key" => hash_value($sessionkey)]);
					if($res){
	...
	function login($formdata = null){
		if($formdata){
			...
			$username = trim($modeldata['username']);
			$password = $modeldata['password'];
			$rememberme = !empty($modeldata['rememberme']);
	
			// Attempt to log in the user
			if($this->login_user($username, $password, $rememberme)){
				// Successful login
				return render_json([
					'success' => true,
